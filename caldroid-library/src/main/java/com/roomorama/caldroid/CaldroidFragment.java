...
public class CaldroidFragment extends DialogFragment {
	...
	public void addSelectedDate(List<Date> dates) {
		for(Date date : dates) {
			selectedDates.add(CalendarHelper.convertDateToDateTime(date));
		}
	}
	...
	private void setupDateGridPages(View view) {
		...
		// Set adapters to the pageChangeListener so it can refresh the adapter when page change
		...
		// Set the numberOfDaysInMonth to dateViewPager so it can calculate the height correctly
		dateViewPager.setDatesInMonth(dateInMonthsList);

		// Prevent pager from save state.
		// When user swipe the pager and change position, the ViewPager save the current position (Virtual Position).
		// and save the in Hierarchy State for future restore.
		// When CaldroidFragment initialize it's view, set the correct month.
		// But after view creation, the ViewPager restore it's state with the "incorrect" Virtual Position.
		dateViewPager.setSaveEnabled(false);

		// MonthPagerAdapter actually provides 4 real fragments. The
		// InfinitePagerAdapter only recycles fragment provided by this MonthPagerAdapter
		final MonthPagerAdapter pagerAdapter = new MonthPagerAdapter(getChildFragmentManager(), R.id.months_infinite_pager);
